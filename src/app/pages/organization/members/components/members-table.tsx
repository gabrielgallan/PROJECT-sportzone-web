...
import {
  ...
  getApiOrganizationsOrganizationslugMembersQueryKey,
...
function MembersTableRow({ member, organizationSlug }: MembersTableRowProps) {
  ...
  async function handleRemoveMember() {
    try {
      await removeMember({ organizationSlug, memberId: member.membership.id });

      await queryClient.invalidateQueries({
        queryKey:
          getApiOrganizationsOrganizationslugMembersQueryKey(organizationSlug),
      });
  ...
  async function handleTransferownership() {
    try {
      ...
      toast(
        `This organization owner now is ${member.user.name ?? member.user.email}`,
      );
    } catch {
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        {format(member.membership.createdAt, "dd MMM yyyy")}
      </TableCell>
...
export function MembersTable() {
  ...
  const { data: membersList } = useGetApiOrganizationsOrganizationslugMembers(
    organizationSlug,
    {
      page: pageIndex,
      limit: "10",
